...
LATEST_INFO="$(github_getLatest Tokisaki-Galaxy webauthn-helper)"

# version check
LATEST_VERSION="$(jq '.tag_name' -r <<< "$LATEST_INFO" | sed 's|^v||')"
[ -n "$LATEST_VERSION" ] || exit 1
RUST_WEBAUTHN_HELPER_VERSION="$(sed -n 's|RUST_WEBAUTHN_HELPER_VERSION:=||p' "$CURDIR/Makefile")"
[ "$RUST_WEBAUTHN_HELPER_VERSION" != "$LATEST_VERSION" ] || exit 0
...
for (( i=0; i<$(jq -r '.assets | length' <<< "$LATEST_INFO"); i++ )); do
	name=$(jq -r '.assets['$i'].name | select(contains("webauthn-helper"))' <<< "$LATEST_INFO")
	[ -n "$name" ] || continue
	...
	arch="$(awk -F'-' '{ printf "%s-%s", $1, $NF }' <<< "$(sed "s|webauthn-helper-$LATEST_VERSION-||;s|\.tar\..*||" <<< "$name")")"
...
sed -i "s|\(RUST_WEBAUTHN_HELPER_VERSION:=\).*|\1$LATEST_VERSION|" "$CURDIR/Makefile"
